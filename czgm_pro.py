...
check_list = [
    ...
    'MzkwODI5NzQ4MQ==','MzkzMzI5Njc0Nw==','Mzg4NzUyMjQxMw=='
]
# 计算sign
def get_sign():
...
""")
    if Btype == "青龙":
        if os.getenv(linxi_token) == None:
            ...
            exit()
        # 变量CK列表
        ck_token = [json.loads(li) if "&" in line else json.loads(line) for line in os.getenv(linxi_token).splitlines() for li in line.split("&")]
    else:
        # 本地CK列表
        ck_token = [
            # 这里填写本地变量
